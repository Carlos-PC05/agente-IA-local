...
if __name__ == "__main__":
    ...
    _real_notes_dir = NOTES_DIR
    with tempfile.TemporaryDirectory() as tmp:
        # Aisla el autochequeo del directorio real de notas (misma
        # convencion que audit_log.py y executor.py con su log temporal).
        import agent.config as _config

        _config.NOTES_DIR = Path(tmp)
        globals()["NOTES_DIR"] = Path(tmp)

        # Directorio vacio al empezar.
        assert list_notes() == "(sin notas)"

        # Camino feliz: crear -> leer -> anadir -> listar -> borrar.
        assert "Error" not in create_note("Comprar leche", "Ojito, no quedan huevos")
        assert "Error" not in read_note("Comprar leche")
        assert "Error" not in append_note("Comprar leche", "Y un pan")
        assert "Ojito" in read_note("Comprar Leche"), "el titulo deberia ignorar mayusculas/espacios"
        assert "pan" in read_note("comprar-leche"), "append deberia anadir al mismo archivo"
        assert "comprar-leche" in list_notes(), "list_notes deberia devolver los nombres sanitizados"

        # create_note sobrescribe; append_note crea si no existe.
        create_note("A", "primera version")
        assert read_note("a") == "primera version"
        assert "Error" not in create_note("A", "version 2")
        assert read_note("a") == "version 2", "create_note deberia sobrescribir"
        assert "Error" not in append_note("B", "solo esta")
        assert "solo esta" in read_note("b")

        # Sanitizacion del titulo.
        assert _sanitize_title("  HOLA mundo  !! ") == "hola-mundo"
        assert _sanitize_title("!!!") == "sin-titulo"

        # Escapes del sandbox de notas: la sanitizacion convierte los ".."
        # y "/" del titulo en guiones, asi que ningun titulo puede apuntar
        # fuera de NOTES_DIR; la nota resultante simplemente no existe.
        assert _sanitize_title("../../config.py") == "config-py"
        assert "no existe" in read_note("../config.py")
        assert "no existe" in read_note("/etc/passwd")

        # Borrar una nota que no existe falla con Error (no excepcion).
        assert "no existe" in delete_note("no-existo")

        # Restaurar el estado real para no romper otros modulos que se
        # importen en el mismo proceso.
        _config.NOTES_DIR = _real_notes_dir
        globals()["NOTES_DIR"] = _real_notes_dir
    ...
    print("OK: agent/tools/notes.py autochequeo pasado")