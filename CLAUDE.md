...
No hay test runner ni linter configurado (no pytest, no ruff/flake8). Cada módulo trae su propio autochequeo ejecutable:

```bash
# Ejecutar el REPL interactivo (requiere Ollama corriendo en localhost:11434 con el modelo cargado)
python main.py

# Autochequeo de un módulo individual (patrón `if __name__ == "__main__"` con asserts)
python -m agent.loop
python -m agent.tools.executor
python -m agent.tools.files
...
Cada autochequeo imprime `OK: <módulo> autochequeo pasado` al final si todo pasa. Al añadir lógica no trivial (rama, bucle, parser) a un módulo, añade o extiende su propio bloque de autochequeo en vez de crear un archivo de test aparte — es la convención establecida en todo `agent/`. La carpeta `tests/golden_tasks/` (Fase 5, aún no implementada) está reservada para tareas end-to-end de regresión contra el modelo real, no para tests unitarios.
